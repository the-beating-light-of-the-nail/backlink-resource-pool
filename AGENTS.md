...
另外：文档头"整理日期"行追加 `(YYYY-MM-DD 新增 …)` 形式的更新备注；新渠道按类型归段（GitHub awesome list → A/B/C，Launch 平台 → D，目录站 → E，中文导航 → F，自家仓 → G，社区/杂项 → H）；用户实测过的渠道在备注列标"已实测"。
...
- J 段（停更/失效档）为不投渠道，仅留档；渠道复活时再移回对应分段。
