...
另外：文档头"整理日期"行追加 `(YYYY-MM-DD 新增 …)` 形式的更新备注；新渠道按类型归段（GitHub awesome list → A/B/C/K/M，Launch 平台 → D，目录站 → E，中文导航 → F，自家仓 → G，社区/杂项 → H）；用户实测过的渠道在备注列标"已实测"。
...
M 段是同日二轮挖掘（jnv/lists 等其它元列表解析 + 主题直搜，1518 个候选仓）的可投渠道，结构与 K 段相同；M3 为排除参考。

**L 段是"列表的列表"来源索引（查源用，不是投稿目标）**：新项目/新类型上线时按 L 段的"查源流程"先查池内 → 再查 L 段各源 → 核验（分区收不收网站、CONTRIBUTING 硬门槛、最近合并时间）→ 结果回填 K/M（可投）或 M3（排除），避免重复检索。双用途渠道（本身既是索引又能投稿，如 DukeLuo/awesome-awesome-prompts）在备注标明、只计一次数。
...
- J 段（停更/失效档）、L 段（来源索引）、K3 与 M3（排除参考）均为不投内容，仅留档；渠道复活或改规则时再移回可投分段。
- 黑名单（26 个）是已确认排除的平台，不进任何推荐。
- **对外投稿文案零自爆（铁律，2026-09-10 起）**：PR/issue/表单文案及项目 README/落地页公开文案里，绝不出现克隆/复刻/仿/参考了/搬运/inspired by/clone of 等表述及原站名、原项目名；内部资料里的项目来源信息只作理解用，一字不外带（细则见 github-backlink 技能铁律 3）。
